...
var angleX = 0; 
var angleY = 0;*/
var angleObj = {angle:0};
var angleXObj = {angle:0};
var angleYObj = {angle:0};
var readInput = true;
...
s.type='text/javascript';document.head.appendChild(s); //if body re-executed
...
$('.angle-label').each(function() {$(this).html(Math.round($(this).data('angle').angle) + " deg")});
$(".rotate-content").css({"-webkit-transform":"rotate("+angleObj.angle+"deg) rotateX(" + angleXObj.angle + "deg) rotateY(" + angleYObj.angle +"deg)"}); }
});
...
$('.angle-slider').each(function() {$(this).slider("value", $(this).data('angle').angle < 0? ((1 - Math.abs($(this).data('angle').angle/360 % 1)) * 100) : (($(this).data('angle').angle/360 % 1) * 100))});
